...
ParserState.prototype.isGetterPropertyReferenced = function (prop) {
	return this.state.References.getter[prop];
};

ParserState.prototype.isSetterPropertyReferenced = function (prop) {
	return this.state.References.setter[prop];
};

ParserState.prototype.isFunctionReferenced = function (prop) {
	return this.state.References.functions[prop];
};
...
function getEncoding (metabase, imports, str, index) {
	...
	switch (ch) {
		...
		case '^': {
			// pointer
			enc = getEncoding(metabase, imports, str, (index || 0) + 1);
			return {
				value: enc.value + ' *',
...
function parseEncoding (metabase, imports, encoding) {
	...
	for (var c = 0; c < argtypes.length; c++) {
		var enc = getEncoding(metabase, imports, argtypes, c);
...
function generateMethod (metabase, imports, cls, classDef, selector, encoding, instance, body) {
	...
	if (body) {
		...
		code.push('\t\t\t' + (isVoid ? '' : 'result_ = ') + '[HyperloopUtils invokeCustomCallback:@[' + args.join(', ') + '] identifier:@"' + identifier + '"];');
...
function encodeFriendlyType (type) {
	switch (type) {
		case 'long': return 'l';
		case 'int': return 'i';
...
function addHyperloopMethodToClass (node, state) {
	...
	if (methodSpec.returnType || methodSpec.arguments) {
		...
		var type = methodSpec.returnType ? encodeFriendlyType(methodSpec.returnType) : 'v';
		var args = methodSpec.arguments;
		if (args) {
			// allow it to be an array of friendly types
			if (Array.isArray(args)) {
				args = args.map(function(arg) {
					return encodeFriendlyType(arg);
				}).join('');
			}
		}
