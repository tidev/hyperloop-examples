...
function generateFromJSON (dir, json, state, callback, includes) {
	...
	generateBuiltins(json, function (err) {
		...
		json.classes.NSObject.methods.extend = {
			...
			name: 'extend',
			impl: function () {
				return 'return Hyperloop.extend(this.$class, arguments[0], arguments[1]);';
			}
		};
		...
		gencustom.generate(dir, state, json);
 ...
 * generate
 */
function generate (dir, fn, custom, callback) {
	fs.readFile(fn, function (err, buf) {
		if (err) { return callback(err); }
		// turn it into JSON
		return generateFromJSON(dir, JSON.parse(buf), custom, callback);
	});
}
