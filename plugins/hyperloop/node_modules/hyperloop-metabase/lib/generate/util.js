...
function getResultWrapper (state, json, obj, instance) {
	...
	switch (type) {
		...
		case 'objc_pointer': {
			...
			if (cls === 'instancetype' || cls === state.class.name || cls.indexOf(state.class.name) === 0) {
				var varname = 'this';
				if (instance) {
					varname += '.constructor';
				}
			...
			if (cls in json.classes) {
				...
				// generate object type (TODO: not sure if this is correct)
				if (value === 'ObjectType' || value === 'objc_pointer') {
					cls = 'NSObject';
					addImport(state, cls, json.classes[cls]);
					return newPrefix(state, cls);
				}
...
function generateMethodBody (state, json, method, preamble, instance, thisobj, argCallback) {
	...
	if (method.returns.type !== 'void') {
		var wrapper = getResultWrapper(state, json, method.returns, instance);
		end = wrapper ? ')' : '';
		result += wrapper;
	}
	...
	method.arguments.forEach(function (arg, i) {
		if (arg.type === 'block') {
			...
			block.arguments.forEach(function (ba, i) {
				// preamble.push('\t\tif (arguments.length > ' +i + ' && arguments[' + i + '] && !Hyperloop.isNull(arguments[' + i + '])) {');
			...
			preamble.push('\t\t_' + name + '.apply(_' + name + ', args);');
			preamble.push('\t};');
			...
			preamble.push('\tthis.$private.' + method.name + '_' + name + ' = _' + name + ';');
			preamble.push('\tthis.$private.' + method.name + '_' + name + 'Callback = _' + name + 'Callback;');
			arg.name = name + 'Block';
	...
	result += '$dispatch(' + thisobj + ', \''+ fnname + '\', ' + arglist +', ' + instance + ')' + end + ';';
	return result;
}
...
function getObjCReturnResult (value, name, returns, asPointer) {
	...
	switch (value.type) {
		...
		case 'obj_interface':
		case 'objc_pointer': {
			return returns + ' (' + name + ' == nil || [' + name + ' isEqual:[NSNull null]]) ? (id)[NSNull null] : (id)[HyperloopPointer pointer:(__bridge void *)' + asPointer + name +' encoding:@encode(id)];';
		}
		case 'Class': {
			return returns + ' (' + name + ' == nil || [(id)' + name + ' isEqual:[NSNull null]]) ? (id)[NSNull null] : (id)[HyperloopPointer pointer:(__bridge void *)' + asPointer + name +' encoding:@encode(Class)];';
		}
		case 'SEL': {
...
function generateInstanceMethod (state, json, method) {
	...
	if (method.arguments.length) {
		method.arguments.forEach(function (arg, i) {
			code.push('\t\t\tthis.$private.' + method.name + '_' + i + ' = arguments[' + i + '];');
		});
	}
...
function findBlock (json, signature, fn) {
	var blocks = json.blocks[fn.framework];
...
function generateObjCArgument (state, json, fn, arg, i, arglist, tab, define) {
	...
	if (isPrimitive(arg.encoding)) {
		...
		arglist.push('(' + n +') ' + name);
		code.push('\t' + (define ? n + ' *' : '') + name + ' = (' + n + '*)[(HyperloopPointer *)' + name + '_ pointerValue];');
	} else if (arg.type === 'objc_pointer' || arg.type === 'id') {
		n = arg.value;
