 ...
 */
function getUserFrameworks (cacheDir, directories, callback) {
	var cacheToken =  crypto.createHash('md5').update(JSON.stringify(directories)).digest('hex');
	var fn = 'metabase-mappings-user-' + cacheToken + '.json';
	if (!fs.existsSync(cacheDir)) {
		fs.mkdirSync(cacheDir);
	}
	var cacheFilename = path.join(cacheDir, fn);
	...
	var result = {};
	async.each(directories, function (dir, cb) {
		async.each(recursiveReadDir(dir), function (fn, cb2) {
			if (/\.h(pp)?$/.test(fn)) {
				...
				var files = result[fw] || {};
				extractImplementations(fn, files);
				result[fw] = files;
			}
			cb2();
		}, cb);
	...
	child.on('exit', function (ec) {
		...
		async.each(fs.readdirSync(buildOutDir), function (fn, cb) {
			if (/\.a$/.test(fn) && fn.indexOf('libPods-') < 0) {
				...
				var dir = path.join(basedir, 'Pods', name, name, 'Resources');
...
function generateCocoaPods (cachedir, basedir, appDir, sdkType, sdkVersion, minSDKVersion, xcodesettings, callback) {
	...
	if (!fs.existsSync(Podfile)) {
		util.loger.debug('No CocoaPods file found');
		return callback();
	}
